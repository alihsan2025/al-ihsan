...
const DEFAULT_VIDEOS: GalleryItem[] = [
  {
    ...
    category: 'Videos',
    title: 'Al-Ihsan Relief in Action',
    description: 'A look at the work Al-Ihsan Relief is doing in the community.',
  ...
  {
    id: 'default-video-2',
    url: 'https://drive.google.com/file/d/1Mlujgr0260Yy-RLHzzUjH_V5rkCGXanR/view?usp=drive_link',
    category: 'Videos',
    title: 'Al-Ihsan Relief Highlights',
    description: 'Highlights from Al-Ihsan Relief programs and outreach.',
    mediaType: 'video_link',
    source: 'videos',
    createdAt: new Date().toISOString(),
  },
  ...
  useEffect(() => {
    const loadMedia = async () => {
      try {
        ...
        setItems(allItems.length > 0 ? allItems : DEFAULT_VIDEOS);

        // Auto-open modal if ?view=id is in the URL
        const searchParams = new URLSearchParams(window.location.search);
        const viewId = searchParams.get('view');
        if (viewId) {
          const itemToView = (allItems.length > 0 ? allItems : DEFAULT_VIDEOS).find(
            i => i.id === viewId || i.id === `vid-${viewId}`
          );
          if (itemToView) {
